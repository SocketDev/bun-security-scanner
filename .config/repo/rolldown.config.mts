...
// Likewise `secrets/find.js` eagerly requires `secrets/keychain.js`, which
// pulls the macos/linux/windows backends (~267KB rendered with their
// vendored dpapi/keytar glue) — but this scanner's ONLY token read is
// `readSocketApiTokenSync({ allowEnvOnly: true })`, and find.js
// short-circuits (`if (allowEnvOnly) return`) before any keychain call — a
// keychain prompt inside `bun install` is unacceptable, so no future call
// site may drop allowEnvOnly either. Stubbing keychain drops the three OS
// backends from the graph automatically. `secrets/broker.js` (2.8KB) is
// deliberately NOT stubbed: stubbing it perturbs rolldown's module dedup so
// two copies of env/rewire collide on one renamed binding inside the sdk's
// pre-bundled region (`require_rewire$1` redeclared; getEnvValue lands
// undefined) — bisected empirically, dist.test.ts catches it.
// Verified reachable-from-THIS-repo only through those eager require chains;
// the built bundle is smoke-tested in every mode by test/dist.test.ts.
const LIB_STUB_PATTERN =
  /@socketsecurity\/lib\/dist\/(?:schema\/validate|secrets\/keychain)\.js$/
