...
- 🚨 A Socket-published pin NEVER moves down; fix the regressed package upstream. The only sanctioned lower pin is a `catalogHolds:` entry in `.config/fleet/pnpm-workspace.fleet.yaml`, which must cascade in the same wave. (`scripts/fleet/check/socket-pins-are-never-lowered.mts`) [`drift-watch`](docs/agents.md/fleet/drift-watch.md)
